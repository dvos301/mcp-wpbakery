...
define( 'MCP_WPBAKERY_VERSION', '0.9.0' );
...
require_once MCP_WPBAKERY_DIR . 'includes/class-site-knowledge.php';
...
add_action(
	...
	function ( $query ) {
		...
		if ( $token && hash_equals( (string) $token, (string) wp_unslash( $_GET['mcp_preview'] ) ) ) {
			...
			// Headless screenshots capture the page without scrolling, so
			// core lazy-loading would leave every below-the-fold image blank.
